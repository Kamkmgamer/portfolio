...
import { ArrowRight, ShieldCheck, Clock, DollarSign, Rocket, Server } from "lucide-react";
...
const caseStudies: Study[] = [
  {
    ...
    category: "SaaS",
    badges: [
      { icon: <Clock className="h-3.5 w-3.5" />, label: "35 days", color: "green" },
      { icon: <DollarSign className="h-3.5 w-3.5" />, label: "$100k+ value", color: "amber" },
      { icon: <ShieldCheck className="h-3.5 w-3.5" />, label: "Security", color: "blue" },
    ],
  ...
  },
  {
    ...
    stack: "Kafka, ClickHouse, Node, Timeseries modeling, edge caching, Grafana.",
    category: "Data Platform",
    badges: [
      { icon: <Server className="h-3.5 w-3.5" />, label: "20M+/day", color: "violet" },
      { icon: <Clock className="h-3.5 w-3.5" />, label: "<200ms", color: "green" },
      { icon: <ShieldCheck className="h-3.5 w-3.5" />, label: "HA/DR", color: "blue" },
    ],
  ...
  },
  {
    ...
    stack: "Next.js App Router, ISR, SEO, A/B testing hooks, PostHog analytics.",
    category: "Growth",
    badges: [
      { icon: <Rocket className="h-3.5 w-3.5" />, label: "+38% CTR", color: "amber" },
      { icon: <Clock className="h-3.5 w-3.5" />, label: "7 days", color: "green" },
      { icon: <ShieldCheck className="h-3.5 w-3.5" />, label: "Core Web Vitals", color: "blue" },
    ],
...
const badgeColor = (c: BadgeColor) =>
  c === "green"
    ...
    : "bg-gradient-to-r from-violet-500 to-purple-600 border-violet-600 text-white shadow-lg shadow-violet-500/25 dark:from-violet-600 dark:to-purple-700 dark:shadow-violet-400/20";
      ...
      >
        {caseStudies.map((study) => (
          <motion.article key={study.title} variants={item}>
            <Card
              className="group h-full overflow-hidden shadow-sm ring-1 ring-black/5 dark:ring-white/5 transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5"
            >
              ...
              <CardContent>
                ...
                <p className="mt-2 text-sm text-muted-foreground">{study.stack}</p>
                ...
                <div className="mt-6">
                  <Link
                    href={study.href}
                    className="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-blue-500 to-purple-600 text-white px-4 py-2 text-sm font-medium shadow hover:opacity-90 transition focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-500"
